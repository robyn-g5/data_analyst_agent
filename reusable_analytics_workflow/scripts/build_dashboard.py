...
def build_html(data, report):
    ...
    :root{{--navy:#17324d;--blue:#236887;--ink:#1f2933;--muted:#62717d;--line:#d9e2e8;--bg:#f5f7f9}}*{{box-sizing:border-box}}body{{margin:0;background:var(--bg);color:var(--ink);font:13px/1.55 -apple-system,BlinkMacSystemFont,"Segoe UI",system-ui,sans-serif;-webkit-font-smoothing:antialiased}}header{{background:var(--navy);color:white;padding:20px max(24px,calc((100% - 1240px)/2))}}header h1{{margin:0;font-size:19px;font-weight:650;letter-spacing:-.01em}}header div{{font-size:12px;opacity:.75;margin-top:4px}}main{{max-width:1240px;margin:auto;padding:20px 24px 40px}}.cards{{display:grid;grid-template-columns:repeat(4,1fr);gap:12px}}.card,.panel,.report{{background:white;border:1px solid var(--line);border-radius:10px;padding:16px}}.card span{{display:block;color:var(--muted);font-size:11.5px}}.card strong{{display:block;font-size:20px;color:var(--navy);margin:6px 0;font-weight:650}}.card em{{display:block;font-style:normal;color:var(--muted);font-size:11.5px;font-weight:600}}.charts{{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-top:14px}}.panel:nth-child(3){{grid-column:1/-1}}h2{{color:var(--navy);font-size:14px;font-weight:650;margin:0 0 10px}}svg{{width:100%;height:auto}}.axis,.value{{font-size:10px;fill:var(--muted)}}.value{{font-weight:650;fill:var(--ink)}}.grid{{stroke:var(--line)}}.legend{{display:flex;gap:14px;flex-wrap:wrap;font-size:11.5px;margin-bottom:8px}}.legend i{{display:inline-block;width:8px;height:8px;margin-right:5px;border-radius:2px}}table{{width:100%;border-collapse:collapse;font-size:12.5px}}th,td{{padding:8px 10px;border-bottom:1px solid var(--line);text-align:left}}th{{font-size:11px;color:var(--muted);font-weight:600}}.report{{margin-top:14px}}.report h2{{margin-top:22px}}.report h2:first-of-type{{margin-top:0}}.report p{{margin:8px 0}}.report li{{margin:5px 0}}.report ul{{padding-left:20px;margin:8px 0}}.report code{{background:var(--bg);padding:1px 5px;border-radius:4px;font-size:11.5px}}@media(max-width:800px){{.cards,.charts{{grid-template-columns:1fr}}.panel:nth-child(3){{grid-column:auto}}}}
